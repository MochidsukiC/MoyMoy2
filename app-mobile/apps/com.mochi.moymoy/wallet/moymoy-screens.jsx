...
function MoyPinStep({ title, sub, summary, notice, seal, onSetupSeal, busy, error,
      ...
      <div style={{ flex: 1, overflow: "auto", padding: "16px 18px 24px" }}>
        ...
        {(seal || onSetupSeal) && (
          <div style={{ marginTop: summary ? 14 : 0 }}>
            <MoySealBadge seal={seal} onSetup={onSetupSeal} />
          </div>
        )}
        {notice && (
...
function MoyPaymentApproval({ pending, intent, loading, error, balance, busy, step, done,
  seal, onSetupSeal, onToPin, onApprove, onDecline, onClose, onCharge, onReturn, onBackToReview }) {
    ...
    <>
      ...
      {step === "pin" && (
        <MoyPinStep zIndex={160} title="PIN で承認" sub="この支払いを実行します"
          summary={summary} seal={seal} onSetupSeal={onSetupSeal}
            ...
            <>
              ...
              <div style={{ marginTop: 16 }}>
                ...
                <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                  maxLength={32} placeholder="ピグルショップ" style={fieldStyle} />
              ...
              <div style={{ marginTop: 16 }}>
                ...
                <input value={form.sub} onChange={(e) => setForm({ ...form, sub: e.target.value })}
                  maxLength={48} placeholder="建材と装飾ブロックのお店" style={fieldStyle} />
              </div>
...
function MoyMoyApp({ onClose, account, accounts = [], onSwitchAccount, onAddAccount, onLogoutAccount,
  ...
  async function doApprove(pin) {
    ...
    try {
      ...
      const stale = ["expired", "already_paid", "not_payable", "merchant_disabled"]
        .indexOf(r.error) >= 0;
      if (stale) {
    ...
    <div className="screen fade-in" style={{ position: "absolute", inset: 0, zIndex: 20,
      ...
      {confirm && pinAsk && (
        <MoyPinStep zIndex={150}
          ...
          seal={seal} onSetupSeal={() => setSealSetup(true)}
          notice="MoyMoy は決済画面以外で PIN を求めません。"
...
Object.assign(window, {
  ...
  MerchantPortal, errLabel, errLabelFor,
});
